...
const createMergedReverseIndex = function (index, db, mode) {
  ...
  return new Promise((resolve, reject) => {
    ...
    Promise.all(indexKeys.map(gracefullGet)).then(currentValues => {
...
const writer = function (docs, db, mode) {
  return new Promise((resolve, reject) => {
    createMergedReverseIndex(createDeltaReverseIndex(docs), db, mode).then(mergedReverseIndex => {
      db.batch(mergedReverseIndex.concat(objectIndex(docs, mode))).then(db.write).then(resolve(docs.map(doc => doc._id)));
    });
  });
};
