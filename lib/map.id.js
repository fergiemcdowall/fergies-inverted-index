module.exports = function (db) {
  ...
  const UNION = function (keys) {
    return new Promise((resolve, reject) => {
      Promise.all(keys.map(key => GET(key))).then(sets => {
        // flatten
  ...
  const EACH = function (keys) {
    return new Promise((resolve, reject) => {
      Promise.all(keys.map(key => db.get(key))).then(result => {
        return resolve(keys.map((key, i) => {
          return {
            prop: key,
            _id: result[i]
          };
  ...
  const RANGE = function (ops) {
    const s = {};
    return new Promise((resolve, reject) => {
      db.createReadStream(ops).on('data', data => data.value.forEach(objectId => {
        s[objectId] = s[objectId] || [];
        s[objectId].push(data.key);
      })).on('end', () => {
        return resolve(Object.keys(s).map(id => {
          return {
            _id: id,
            prop: s[id]
          };
